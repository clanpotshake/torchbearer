import { DisplayClass, TBItem, TBItemData } from './TBItem';
import { Ability, TBActor, TracksTests } from '../actors/TBActor';
import { StringIndexedObject, TestString, updateTestsNeeded } from "../helpers";
import { skillImages, SkillTypeString } from "../constants";

export class Skill extends TBItem<SkillDataRoot> {
  getRootSelect(): StringIndexedObject<string> {
    const roots = {};
    const actor = this.actor as unknown as TBActor | null;
  ...
  calculateAptitude(this: Skill): void {
    const actor = this.actor as TBActor | null;
    if (!actor || !this.data.hasOwner) {
      return;
    }
    let aptitudeMod =
      actor.getAptitudeModifiers(this.name) + actor.getAptitudeModifiers(this.data.data.skilltype);

    aptitudeMod += actor.getAptitudeModifiers(this.data.data.root1);

    if (this.data.data.root2) {
      aptitudeMod +=
        actor.getAptitudeModifiers(`${this.data.data.root1}/${this.data.data.root2}`) +
  ...
  //       actor: this.actor as unknown as TBActor,
  //       skill: this,
  //       difficulty,
  //       name: this.name,
  //     });
...
export interface SkillDataRoot extends TBItemData<SkillData> {
  type: 'skill';
}
