...
import { ItemType } from '../items/TBItem';
import { CreateEditMacro, RollEditMacro } from './EditMacro';
import { CreateSkillRollMacro, RollSKillMacro } from './SkillMacro';

export async function CreateTorchbearerMacro(data: DragData, slot: string): Promise<boolean> {
  ...
  if (macroData) {
    ...
    let macro = game.macros?.contents.find(
      (m) => m.name === macroData.name && (m.data as any).command === macroData.command,
    );
    if (!macro) {
...
export function RegisterMacros(): void {
  game.torchbearer.macros = game.torchbearer.macros || {};
  game.torchbearer.macros['rollSkill'] = RollSKillMacro;
  // game.torchbearer.macros['rollMelee'] = RollMeleeMacro;
  // game.torchbearer.macros['rollRanged'] = RollRangedMacro;
  // game.torchbearer.macros['rollSpell'] = RollSpellMacro;
  game.torchbearer.macros['showOwnedItem'] = RollEditMacro;
  game.torchbearer.macros['rollStat'] = RollStatMacro;
}
...
/* ============== Macro Helpers =============== */
export function getMacroRollPreset(actor: TBActor): Partial<RollDialogData> {
  const dataPreset: Partial<RollDialogData> = {};
  if (game.settings.get(constants.systemName, constants.settings.useGmDifficulty)) {
    const difficultyDialog: DifficultyDialog = game.torchbearer.gmDifficulty;
    const helpDialog: ModifierDialog = game.torchbearer.modifiers;
