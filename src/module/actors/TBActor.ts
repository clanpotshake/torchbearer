import { DisplayClass, ItemType, TBItem, TBItemData, TBItemDataTypes } from '../items/TBItem';
import * as constants from '../constants.js';
import { StringIndexedObject } from '../helpers';
import { SkillDataRoot } from '../items/TBSkill';
import { TBCharacterData } from './sheets/TBCharacter';
import {
  ActorData,
  ActorDataSchema
} from "@league-of-foundry-developers/foundry-vtt-types/src/foundry/common/data/data.mjs/actorData";

export class TBActor<T extends TBActorData = TBActorData> extends Actor {
  data: T;

  readonly batchAdd = {
    task: -1,
    items: [] as (NewItemData & Partial<TBItemDataTypes>)[],
  };
  ...
  private _prepareActorData() {
    ...
    if (this.data.items) {
      this.data.items.forEach(({ data }) => {
        const i: TBItemData = data;
        switch (i.type) {
          case 'skill':
            if (!(i as SkillDataRoot).data.learning && !(i as SkillDataRoot).data.training) {
  ...
  _onCreate(data: any, options: any, userId: string): void {
    ...
    this.createEmbeddedDocuments('Item', [
      { name: 'Belief', type: 'belief', data: {}, img: constants.defaultImages.belief },
      { name: 'Belief Special', type: 'belief', data: {}, img: constants.defaultImages.belief },
      { name: 'Instinct', type: 'instinct', data: {}, img: constants.defaultImages.instinct },
      {
      ...
      { name: 'Goal', type: 'goal', data: {}, img: constants.defaultImages.goal },
      { name: 'Goal Special', type: 'goal', data: {}, img: constants.defaultImages.goal },
    ]);
  }
  ...
  private _calculateClumsyWeight() {
    ...
    const charData =
      this.data.type === 'character' ? (this.data.data as TBCharacterData) : undefined;

    this.data.items
      .filter<Armor>((i) => i.type === 'armor' && i.data.data.equipped)
      .forEach((i) => {
        const a = i.data;
        if (a.data.hasHelm) {
...
export interface TBActorData<T extends Common = Common> extends ActorData {
